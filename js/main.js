...
document.getElementById('btn-start').addEventListener('click',(e)=>{e.stopPropagation();startGame();});document.getElementById('start-overlay').addEventListener('click',startGame);document.getElementById('btn-reset-world').addEventListener('click',(e)=>{e.stopPropagation();if(confirm('ワールドを初期化しますか？建築した内容はすべて消えます。')){localStorage.removeItem('bw_edits');localStorage.removeItem('bw_seed');localStorage.removeItem('bw_inventory');location.reload();}});engine.runRenderLoop(()=>{const dt=Math.min(0.05,engine.getDeltaTime()/1000);update(dt);if(typeof FLUID!=='undefined')FLUID.update(dt);updateDayNight(dt);updateHUD(dt);scene.render();});function setAppHeight(){const vv=window.visualViewport,h=vv?vv.height:window.innerHeight,top=vv?vv.offsetTop:0;const bottom=Math.max(0,window.innerHeight-h-top);const root=document.documentElement.style;root.setProperty('--app-height',h+'px');root.setProperty('--vv-top',top+'px');root.setProperty('--vv-bottom',bottom+'px');engine.resize();}
