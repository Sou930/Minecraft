...
function updateAudioEnvironment(dt){if(typeof SFX==='undefined')return;const px=Math.floor(player.pos.x),py=Math.floor(player.pos.y+1),pz=Math.floor(player.pos.z);const underground=!(typeof skyExposed==='function'&&skyExposed(px,py,pz));let nearWater=false;const r=4;outer:for(let dx=-r;dx<=r;dx++)for(let dy=-2;dy<=2;dy++)for(let dz=-r;dz<=r;dz++){const b=getBlock(px+dx,py+dy,pz+dz);if(b===B.WATER){nearWater=true;break outer;}}
