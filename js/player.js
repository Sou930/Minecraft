...
// 立ち上がる/姿勢を高くする場合は頭上に空間があるか確認する。
function togglePose(target){if(player.flying)return;const next=(player.pose===target)?POSE.STAND:target;if(next<player.pose){
  // より高い姿勢（数値が小さい）へ戻すときは頭上スペースを確認
  if(!poseFits(next)){return;}
}
player.pose=next;applyPose();updatePoseUI();}
function updatePoseUI(){const el=document.getElementById('pose-display');if(!el)return;const labels=['','🦵 しゃがみ','🐍 匍匐'];el.textContent=labels[player.pose]||'';el.style.display=player.pose===POSE.STAND?'none':'block';}
// ===== 視点（1人称 / 3人称背面 / 3人称正面）=====
const VIEW={FIRST:0,THIRD_BACK:1,THIRD_FRONT:2};
let cameraView=VIEW.FIRST;
const VIEW_NAMES=['1人称','3人称(背面)','3人称(正面)'];
