...
OBSERVER_FACE:207,OBSERVER_BACK:208,OBSERVER_SIDE:209,
...
// Sky Island
SKY_STONE:255,
// ---- New decorative blocks (row 32-39) ----
FALLEN_LEAVES:256,
PINE_CONE:257,
MARBLE:258,
BLACK_MARBLE:259,
SMOOTH_STONE:260,
SANDSTONE_TILE:261,
BASALT_PILLAR_TOP:262,
BASALT_PILLAR_SIDE:263,
// ---- Nether tiles ----
NETHERRACK_TILE:264,
GLOWSTONE_TILE:265,
QUARTZ_ORE_TILE:266,
QUARTZ_BLOCK_TILE:267,
SOUL_SAND_TILE:268,
NETHER_PORTAL_TILE:269,
NETHER_WART_TILE:270,
// ---- Deep Dark / Sculk tiles ----
SCULK_TILE:271,
SCULK_VEIN_TILE:272,
SCULK_CATALYST_TILE:273,
SCULK_SHRIEKER_TILE:274,
// ---- End tiles ----
END_STONE_TILE:275,
END_STONE_BRICK_TILE:276,
PURPUR_TILE:277,
CHORUS_PLANT_TILE:278,
END_PORTAL_TILE:279,
DRAGON_EGG_TILE:280,
...
HOPPER:205,
OBSERVER:206,
// New decorative / interactive blocks
SIGN:207,
ITEM_FRAME:208,
FLOWER_POT:209,
IRON_BARS:210,
...
// ---- Sign (看板) ----
BLOCKS[B.SIGN]={name:'Sign',all:T.SIGN_BOARD,transparent:true,sign:true,breakTime:1.0,signText:'',signFacing:0};
// ---- Item Frame (額縁) ----
BLOCKS[B.ITEM_FRAME]={name:'Item Frame',all:T.ITEM_FRAME,transparent:true,itemFrame:true,breakTime:0.5,frameItem:null,frameFacing:0};
// ---- Flower Pot (花瓶・植木鉢) ----
BLOCKS[B.FLOWER_POT]={name:'Flower Pot',all:T.FLOWER_POT,transparent:true,flowerPot:true,breakTime:0.5,potPlant:0};
// ---- Iron Bars (鉄格子) ----
BLOCKS[B.IRON_BARS]={name:'Iron Bars',all:T.IRON_BARS,transparent:true,ironBars:true,breakTime:3.0,toolClass:'pickaxe'};
// ---- Glass Pane (ガラス板) ----
BLOCKS[B.GLASS_PANE]={name:'Glass Pane',all:T.GLASS,transparent:true,glassPane:true,breakTime:0.45};
// ---- Wall torches (6-direction torch) ----
BLOCKS[B.TORCH_WALL_N]={name:'Torch',all:T.TORCH,transparent:true,torchWall:true,torchFacing:'N',breakTime:0.0,emissive:true};
BLOCKS[B.TORCH_WALL_S]={name:'Torch',all:T.TORCH,transparent:true,torchWall:true,torchFacing:'S',breakTime:0.0,emissive:true};
BLOCKS[B.TORCH_WALL_E]={name:'Torch',all:T.TORCH,transparent:true,torchWall:true,torchFacing:'E',breakTime:0.0,emissive:true};
BLOCKS[B.TORCH_WALL_W]={name:'Torch',all:T.TORCH,transparent:true,torchWall:true,torchFacing:'W',breakTime:0.0,emissive:true};
BLOCKS[B.TORCH_CEILING]={name:'Torch',all:T.TORCH,transparent:true,torchCeiling:true,breakTime:0.0,emissive:true};
// ---- New ores / rocks / mushrooms ----
// Emerald ore (rare, mountain-biome stone)
BLOCKS[B.EMERALD_ORE]={name:'Emerald Ore',all:T.EMERALD_ORE,breakTime:11.0,toolClass:'pickaxe',ore:true};
// Ruby ore (custom red gem, deep underground)
BLOCKS[B.RUBY_ORE]={name:'Ruby Ore',all:T.RUBY_ORE,breakTime:11.0,toolClass:'pickaxe',ore:true};
// Sapphire ore (custom blue gem, deep underground)
BLOCKS[B.SAPPHIRE_ORE]={name:'Sapphire Ore',all:T.SAPPHIRE_ORE,breakTime:11.0,toolClass:'pickaxe',ore:true};
// Obsidian ore (rare in-ground block, deep / near lava)
BLOCKS[B.OBSIDIAN_ORE]={name:'Obsidian Ore',all:T.OBSIDIAN_ORE,breakTime:22.0,toolClass:'pickaxe',ore:true};
// Sulfur block (volcanic, bright yellow crystalline)
BLOCKS[B.SULFUR_BLOCK]={name:'Sulfur Block',all:T.SULFUR_BLOCK,breakTime:3.0,toolClass:'pickaxe'};
// Phosphor stone (glows in the dark — a GLOW_LICHEN stone tablet)
BLOCKS[B.PHOSPHOR_STONE]={name:'Phosphor Stone',all:T.PHOSPHOR_STONE,breakTime:4.0,toolClass:'pickaxe',emissive:true};
// Polished amethyst (a refined AMETHYST_BLOCK)
BLOCKS[B.POLISHED_AMETHYST]={name:'Polished Amethyst',all:T.POLISHED_AMETHYST,breakTime:6.0,toolClass:'pickaxe',emissive:true};
// Lead ore (heavy — affects player fall speed when stood on)
BLOCKS[B.LEAD_ORE]={name:'Lead Ore',all:T.LEAD_ORE,breakTime:10.0,toolClass:'pickaxe',ore:true,heavy:true};
// Tin ore (used with iron to craft bronze)
BLOCKS[B.TIN_ORE]={name:'Tin Ore',all:T.TIN_ORE,breakTime:9.0,toolClass:'pickaxe',ore:true};
// Bronze block (crafted from iron + tin)
BLOCKS[B.BRONZE_BLOCK]={name:'Bronze Block',all:T.BRONZE_BLOCK,breakTime:12.0,toolClass:'pickaxe'};
// Lava rock (cooled-lava textured rock, volcano biome)
BLOCKS[B.LAVA_ROCK]={name:'Lava Rock',all:T.LAVA_ROCK,breakTime:8.0,toolClass:'pickaxe'};
// Tuff (dark volcanic rock, deep underground)
BLOCKS[B.TUFF]={name:'Tuff',all:T.TUFF,breakTime:7.5,toolClass:'pickaxe'};
// Deepslate (very dark stone for the deepest caves)
BLOCKS[B.DEEPSLATE]={name:'Deepslate',all:T.DEEPSLATE,breakTime:9.0,toolClass:'pickaxe'};
// Huge mushrooms (red & brown) — grown in jungle/swamp
BLOCKS[B.HUGE_MUSHROOM_RED]={name:'Red Mushroom',all:T.HUGE_MUSHROOM_RED,breakTime:0.4,transparent:true,crossPlant:true,mushroom:true};
BLOCKS[B.HUGE_MUSHROOM_BROWN]={name:'Brown Mushroom',all:T.HUGE_MUSHROOM_BROWN,breakTime:0.4,transparent:true,crossPlant:true,mushroom:true};
// Mushroom block (mycelium block — the cap of a giant mushroom)
...
// ---- New decorative blocks ----
// 紅葉した落ち葉ブロック（敷物化） — flat carpet of autumn fallen leaves
BLOCKS[B.FALLEN_LEAVES]={name:'Fallen Leaves',all:T.FALLEN_LEAVES,transparent:true,flat:true,breakTime:0.2};
// 松ぼっくり装飾ブロック — pine cone cross decoration
BLOCKS[B.PINE_CONE]={name:'Pine Cone',all:T.PINE_CONE,transparent:true,crossPlant:true,breakTime:0.3};
// 大理石ブロック（白・装飾用）— white marble block
BLOCKS[B.MARBLE]={name:'Marble',all:T.MARBLE,breakTime:8.0,toolClass:'pickaxe'};
// 黒大理石ブロック — black marble block
BLOCKS[B.BLACK_MARBLE]={name:'Black Marble',all:T.BLACK_MARBLE,breakTime:8.0,toolClass:'pickaxe'};
// 滑らかな石（SMOOTH_STONE）— polished smooth stone
BLOCKS[B.SMOOTH_STONE]={name:'Smooth Stone',all:T.SMOOTH_STONE,breakTime:6.0,toolClass:'pickaxe'};
// 砂岩タイル — chiseled sandstone tile
BLOCKS[B.SANDSTONE_TILE]={name:'Sandstone Tile',all:T.SANDSTONE_TILE,breakTime:4.0,toolClass:'pickaxe'};
// 玄武岩の柱（円柱状）— basalt pillar column
BLOCKS[B.BASALT_PILLAR]={name:'Basalt Pillar',top:T.BASALT_PILLAR_TOP,side:T.BASALT_PILLAR_SIDE,bottom:T.BASALT_PILLAR_TOP,breakTime:9.0,toolClass:'pickaxe'};
...
(function tagBlockTools(){
  const pick=[B.STONE,B.COBBLE,B.BRICK,B.STONE_BRICK,B.MOSSY_BRICK,B.CRACKED_BRICK,B.COAL_ORE,B.IRON_ORE,
    ...
    B.MARBLE,B.BLACK_MARBLE,B.SMOOTH_STONE,B.SANDSTONE_TILE,B.BASALT_PILLAR,
...
const STACK_MAX=64;
// Tools do not stack
function isArmorLike(id){return !!(ITEMS[id]&&(ITEMS[id].armor||ITEMS[id].shield||ITEMS[id].ranged));}
function maxStackOf(id){return (isTool(id)||isArmorLike(id))?1:STACK_MAX;}function dropFor(id){if(id===B.GRASS)return Math.random()<0.18?ITEM_SEEDS:B.DIRT;if(id===B.STONE)return B.COBBLE;if(id===B.GLASS)return null;if(id===B.ICE)return null;if(id===B.LAVA)return null;if(id===B.LEAVES||id===B.BIRCH_LEAVES||id===B.MANGROVE_LEAVES||id===B.PALM_LEAVES||id===B.MAPLE_LEAVES_RED||id===B.MAPLE_LEAVES_ORANGE||id===B.MAPLE_LEAVES_YELLOW)return Math.random()<0.2?ITEM_APPLE:null;if(id===B.COBWEB)return null;if(id===B.PATH)return B.DIRT;if(id===B.GLOW_LICHEN)return null;if(id===B.FARMLAND||id===B.FARMLAND_WET)return B.DIRT;if(id===B.MELON)return null;if(id===B.DEAD_BUSH)return null;if(id===B.TALL_GRASS)return Math.random()<0.125?ITEM_SEEDS:null;if(id===B.STAIRS_E||id===B.STAIRS_S||id===B.STAIRS_W)return B.STAIRS_N;
...
// Wall/ceiling torches drop a regular torch
if(id===B.TORCH_WALL_N||id===B.TORCH_WALL_S||id===B.TORCH_WALL_E||id===B.TORCH_WALL_W||id===B.TORCH_CEILING)return B.TORCH;
// Glass pane drops nothing (like glass)
if(id===B.GLASS_PANE)return null;
return id;}
...
// ---- Sign: 6 planks (2 rows) + 1 stick → 3 signs ----
{cat:'deco',pattern:[[B.PLANKS,B.PLANKS,B.PLANKS],[B.PLANKS,B.PLANKS,B.PLANKS],[null,ITEM_STICK,null]],out:{id:B.SIGN,count:3}},
// ---- Item Frame: 8 sticks around 1 leather → 1 frame ----
{cat:'deco',pattern:[[ITEM_STICK,ITEM_STICK,ITEM_STICK],[ITEM_STICK,ITEM_LEATHER,ITEM_STICK],[ITEM_STICK,ITEM_STICK,ITEM_STICK]],out:{id:B.ITEM_FRAME,count:1}},
// ---- Flower Pot: 3 bricks in U shape → 1 pot ----
{cat:'deco',pattern:[[B.BRICK,null,B.BRICK],[null,B.BRICK,null]],out:{id:B.FLOWER_POT,count:1}},
// ---- Iron Bars: 6 iron in 2 rows → 16 bars ----
...
// ---- Bronze: iron + tin combined → 1 bronze block ----
{cat:'building',pattern:[[B.IRON_ORE,B.TIN_ORE],[B.TIN_ORE,B.IRON_ORE]],out:{id:B.BRONZE_BLOCK,count:1}},
// ---- Polished Amethyst: 4 amethyst blocks → 4 polished ----
{cat:'deco',pattern:[[B.AMETHYST_BLOCK,B.AMETHYST_BLOCK],[B.AMETHYST_BLOCK,B.AMETHYST_BLOCK]],out:{id:B.POLISHED_AMETHYST,count:4}},
// ---- Lava Rock: 4 smooth basalt + 1 lava-rock proxy → cooled lava rock ----
{cat:'building',pattern:[[B.SMOOTH_BASALT,B.SMOOTH_BASALT],[B.SMOOTH_BASALT,B.LAVA]],out:{id:B.LAVA_ROCK,count:4}},
// ---- Mushroom block: 4 huge mushrooms (red) → 1 mushroom block ----
...
// ---- New decorative block recipes ----
// 紅葉した落ち葉（敷物）: maple leaves → fallen leaves carpet
{cat:'deco',pattern:[[B.MAPLE_LEAVES_RED,B.MAPLE_LEAVES_ORANGE],[B.MAPLE_LEAVES_YELLOW,B.MAPLE_LEAVES_RED]],out:{id:B.FALLEN_LEAVES,count:4}},
// 松ぼっくり: spruce leaves + spruce log → pine cone
{cat:'deco',pattern:[[B.SPRUCE_LEAVES],[B.SPRUCE_LOG]],out:{id:B.PINE_CONE,count:2}},
// 大理石（白）: 4 calcite → 4 marble
{cat:'building',pattern:[[B.CALCITE,B.CALCITE],[B.CALCITE,B.CALCITE]],out:{id:B.MARBLE,count:4}},
// 黒大理石: marble + obsidian → black marble
{cat:'building',pattern:[[B.MARBLE,B.OBSIDIAN],[B.OBSIDIAN,B.MARBLE]],out:{id:B.BLACK_MARBLE,count:2}},
// 滑らかな石: 4 stone → smooth stone (polished)
{cat:'building',pattern:[[B.STONE,B.STONE],[B.STONE,B.STONE]],out:{id:B.SMOOTH_STONE,count:4}},
// 砂岩タイル: sandstone + chisel (use stone brick as proxy) → sandstone tile
{cat:'building',pattern:[[B.SANDSTONE,B.SANDSTONE,B.SANDSTONE],[B.SANDSTONE,B.STONE_BRICK,B.SANDSTONE]],out:{id:B.SANDSTONE_TILE,count:4}},
// 玄武岩の柱: 2 smooth basalt vertically → basalt pillar
{cat:'building',pattern:[[B.SMOOTH_BASALT],[B.SMOOTH_BASALT]],out:{id:B.BASALT_PILLAR,count:2}},
// つる植物（壁に這う）: leaves + moss → vine
{cat:'deco',pattern:[[B.LEAVES],[B.MOSS]],out:{id:B.VINE,count:4}},
];
...
const ACHIEVEMENTS=[
  ...
  {id:'nether_portal',icon:'🔥',name:'Nether Bound',desc:'Ignite a Nether Portal',stat:'nether_portal',goal:1},
  {id:'deep_dark_warden',icon:'💀',name:'Warden Encounter',desc:'Encounter the Warden in the Deep Dark',stat:'deep_dark_warden',goal:1},
  {id:'warden_defeated',icon:'⚔',name:'Warden Slayer',desc:'Defeat the Warden',stat:'warden_defeated',goal:1},
  {id:'dragon_slain',icon:'🐉',name:'Ender Dragon Slain',desc:'Defeat the Ender Dragon',stat:'dragon_slain',goal:1},
];
