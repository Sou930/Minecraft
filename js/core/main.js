...
if(worldReady&&started&&typeof REDSTONE!=='undefined'){REDSTONE.update(dt);REDSTONE.updateHoppers(dt);}updateDayNight(dt);if(worldReady){if(typeof processRelightQueue!=='undefined')processRelightQueue(3);if(typeof LIGHTING!=='undefined')LIGHTING.update(dt);if(typeof SHADERFX!=='undefined'){const dayF=(typeof LIGHTING!=='undefined')?LIGHTING.getNightFactor():1;SHADERFX.update(dt,dayF);}updateAudioEnvironment(dt);if(typeof updatePetals==='function')updatePetals(dt);if(typeof updateWindmills==='function')updateWindmills(dt);if(typeof PERF!=='undefined')PERF.update(dt);}
